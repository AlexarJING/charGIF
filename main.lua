...
require "love.math"
local example = "example/demo.gif"
local matcat = require "matcat".new("matrix.txt")
--matcat.getDensity()
...
local gifnew = require 'gifloader.gifload.gifload'
local twimer = require 'gifloader.twimer.twimer'()
...
local function usage()
  print("Usage: love vgif.love [--flatbg] [--vsync] [--autosize] image.gif")
end
...
function love.load(args)
  ...
  preMakeCanvas = love.graphics.newCanvas()
  bigCanvas = love.graphics.newCanvas(love.graphics.getWidth()*5, love.graphics.getWidth()*5)
...
function love.draw()
  love.graphics.clear()
  love.graphics.setCanvas(preMakeCanvas)
  love.graphics.clear()
  if checker then
  ...
  --love.graphics.draw(preMakeCanvas,0,0,0,5,5)
  love.graphics.setCanvas(bigCanvas)
  love.graphics.draw(preMakeCanvas,0,0,0,1,1)
  love.graphics.setCanvas()

  love.graphics.draw(bigCanvas,300,0)

  local data = bigCanvas:newImageData()
  love.graphics.setColor(255, 255, 255, 255)
  for x = 1, bigCanvas:getWidth()-5,5 do
    for y = 1, bigCanvas:getHeight()-7,7 do
      local r,g,b,a = data:getPixel(x+2,y+3)
      local gray = math.ceil(r + g + b)/3
      
      if gray == 0  then
        matcat.print(" ",x,y,1)
      else
        local chars = matcat.density[math.floor(gray*145/255)]
        matcat.print(chars[love.math.random(#chars)],x,y,1)
      end
    end
  end
end
